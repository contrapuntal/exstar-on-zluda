...
impl DarkApiState2 {
    ...
    fn export_table_size_matches(
        our_functions: &::dark_api::DarkApiTable<'_>,
        export_functions_start_idx: usize,
        export_functions_size: usize,
    ) -> bool {
        if our_functions.len() == export_functions_size {
            return true;
        }
        if export_functions_start_idx != 0 || our_functions.len() < export_functions_size {
            return false;
        }
        (export_functions_size..our_functions.len()).all(|idx| our_functions.get_fn(idx).is_none())
    }
    ...
    pub(crate) fn override_export_table(
        ...
        if let Some(ref our_functions) = our_functions {
            if !Self::export_table_size_matches(
                our_functions,
