...
use crate::r#impl::{debug, driver, hipfix};
...
pub(crate) unsafe fn get_attribute(
    ...
    debug::log_stream_memory(format_args!(
        "op=cuPointerGetAttribute phase=enter attribute={} ptr={:?} data={:p}",
        attribute.0, ptr.0, data
    ));
    match attribute {
        hipPointer_attribute::HIP_POINTER_ATTRIBUTE_CONTEXT => {
            ...
            debug::log_stream_memory(format_args!(
                "op=cuPointerGetAttribute phase=return attribute={} ptr={:?} context={:?}",
                attribute.0, ptr.0, context
            ));
            Ok(())
        }
        hipPointer_attribute::HIP_POINTER_ATTRIBUTE_MEMORY_TYPE => {
            ...
            debug::log_stream_memory(format_args!(
                "op=cuPointerGetAttribute phase=return attribute={} ptr={:?} hip_memory_type={} cu_memory_type={}",
                attribute.0,
                ptr.0,
                memory_type.0,
                (*(data.cast::<CUmemorytype>())).0
            ));
            Ok(())
        }
        _ => {
            ...
            debug::log_stream_memory(format_args!(
                "op=cuPointerGetAttribute phase=return attribute={} ptr={:?} data={:p}",
                attribute.0, ptr.0, data
            ));
            Ok(())
        }
...
pub(crate) unsafe fn get_attributes(
    ...
    debug::log_stream_memory(format_args!(
        "op=cuPointerGetAttributes phase=enter num_attributes={} ptr={:?} attributes_ptr={:p} data_ptr={:p}",
        num_attributes,
        ptr.0,
        attributes,
        data
    ...
    debug::log_stream_memory(format_args!(
        "op=cuPointerGetAttributes phase=return num_attributes={} ptr={:?}",
        num_attributes, ptr.0
    ));
    Ok(())
}
