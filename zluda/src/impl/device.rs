use super::{context, debug, driver};
use cuda_types::cuda::*;
use hip_runtime_sys::*;
use std::mem;

const EXSTAR_SPOOFED_DEVICE_NAME: &[u8] = b"NVIDIA GeForce RTX 3060";
const EXSTAR_SPOOFED_TOTAL_MEM_BYTES: usize = 12 * 1024 * 1024 * 1024;
...
fn exstar_spoofed_attribute(attrib: CUdevice_attribute) -> Option<i32> {
    match attrib {
        CUdevice_attribute::CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR => Some(8),
        CUdevice_attribute::CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR => Some(6),
        CUdevice_attribute::CU_DEVICE_ATTRIBUTE_INTEGRATED => Some(0),
        CUdevice_attribute::CU_DEVICE_ATTRIBUTE_MEMORY_CLOCK_RATE => Some(7_500_000),
        CUdevice_attribute::CU_DEVICE_ATTRIBUTE_GLOBAL_MEMORY_BUS_WIDTH => Some(192),
        _ => None,
    }
}

unsafe fn write_spoofed_device_name(
    ...
    let dst = std::slice::from_raw_parts_mut(name.cast::<u8>(), len as usize);
    if dst.is_empty() {
        return Err(CUerror::INVALID_VALUE);
    }
    let copy_len = usize::min(
        dst.len().saturating_sub(1),
        EXSTAR_SPOOFED_DEVICE_NAME.len(),
    );
    dst[..copy_len].copy_from_slice(&EXSTAR_SPOOFED_DEVICE_NAME[..copy_len]);
    dst[copy_len] = 0;
    Ok(())
}
...
pub(crate) fn get_attribute(
    ...
    debug::log_launch(format_args!(
        "phase=device_query_enter op=cuDeviceGetAttribute attrib={attrib:?} dev={dev_idx}"
    ));
    if debug::exstar_device_qualification_compat() {
        if let Some(value) = exstar_spoofed_attribute(attrib) {
            *pi = value;
            debug::log_launch(format_args!(
                "phase=device_query_return op=cuDeviceGetAttribute attrib={attrib:?} dev={dev_idx} value={} result={:?} spoof=exstar_device_qualification",
                value,
                CUresult::SUCCESS
            ));
            return Ok(());
    ...
    let result = unsafe { hipDeviceGetAttribute(pi, attrib, dev_idx) };
    let value = *pi;
    debug::log_launch(format_args!(
        "phase=device_query_return op=cuDeviceGetAttribute attrib={attrib:?} dev={dev_idx} value={} result={:?}",
        value,
        result
    ));
    result?;
...
pub(crate) fn get_name(
    ...
    debug::log_launch(format_args!(
        "phase=device_query_enter op=cuDeviceGetName dev={dev} len={len}"
    ));
    if debug::exstar_device_qualification_compat() {
        unsafe { write_spoofed_device_name(name, len) }?;
        let spoofed_name = unsafe { std::ffi::CStr::from_ptr(name) }.to_string_lossy();
        debug::log_launch(format_args!(
            "phase=device_query_return op=cuDeviceGetName dev={dev} name={} spoof=exstar_device_qualification",
            spoofed_name
        ));
        return Ok(());
    }
    ...
    debug::log_launch(format_args!(
        "phase=device_query_return op=cuDeviceGetName dev={dev} name={}",
        name
    ));
    Ok(())
}
...
pub(crate) fn total_mem_v2(bytes: *mut usize, dev: hipDevice_t) -> hipError_t {
    debug::log_launch(format_args!(
        "phase=device_query_enter op=cuDeviceTotalMem_v2 dev={dev}"
    ));
    if debug::exstar_device_qualification_compat() {
        if let Some(bytes) = unsafe { bytes.as_mut() } {
            *bytes = EXSTAR_SPOOFED_TOTAL_MEM_BYTES;
        }
        debug::log_launch(format_args!(
            "phase=device_query_return op=cuDeviceTotalMem_v2 dev={dev} bytes={} result={:?} spoof=exstar_device_qualification",
            EXSTAR_SPOOFED_TOTAL_MEM_BYTES,
            hipError_t::Ok(())
        ));
        return Ok(());
    }
    let result = unsafe { hipDeviceTotalMem(bytes, dev) };
    let value = unsafe { bytes.as_ref().copied().unwrap_or(0) };
    debug::log_launch(format_args!(
        "phase=device_query_return op=cuDeviceTotalMem_v2 dev={dev} bytes={} result={:?}",
        value, result
    ));
    result
}
...
pub(crate) fn get_properties(prop: &mut CUdevprop, dev: hipDevice_t) -> hipError_t {
    debug::log_launch(format_args!(
        "phase=device_query_enter op=cuDeviceGetProperties dev={dev}"
    ));
    ...
    debug::log_launch(format_args!(
        "phase=device_query_return op=cuDeviceGetProperties dev={dev} maxThreadsPerBlock={} memPitch={} clockRate={} textureAlign={}",
...
pub(crate) fn get_count(count: &mut ::core::ffi::c_int) -> hipError_t {
    debug::log_launch(format_args!("phase=device_query_enter op=cuDeviceGetCount"));
    let result = unsafe { hipGetDeviceCount(count) };
    let value = *count;
    debug::log_launch(format_args!(
        "phase=device_query_return op=cuDeviceGetCount count={} result={:?}",
        value, result
    ));
    result
}
